...
class ReviewRateLimiter:
    ...
    def _cleanup_old_requests(
        ...
        five_min_ago = now - timedelta(minutes=5)
        user_data["requests"] = [
            ts for ts in user_data["requests"] if ts > five_min_ago
        ]
    ...
    def _check_and_ban(
        ...
        if user_data["violation_count"] >= self.BAN_THRESHOLD_VIOLATIONS:
            ...
            user_data["ban_reason"] = (
                f"Excessive violations ({user_data['violation_count']} times)"
            )
            user_data["ban_timestamp"] = now
    ...
    def get_user_status(
        ...
        requests_1min = [
            ts for ts in user_data["requests"] if ts > now - timedelta(minutes=1)
        ]
        requests_5min = [
            ts for ts in user_data["requests"] if ts > now - timedelta(minutes=5)
        ]
                ...
                banned.append(
                    {
                        ...
                        "ban_timestamp": data["ban_timestamp"].isoformat()
                        if data["ban_timestamp"]
                        else None,
    ...
    def get_user_ban_info(
        ...
        return {
            ...
            "banned": True,
            "ban_reason": user_data["ban_reason"],
            "ban_timestamp": user_data["ban_timestamp"].isoformat()
            if user_data["ban_timestamp"]
            else None,
            "violation_count": user_data["violation_count"],
            "requests_count": len(user_data["requests"]),
            "last_request": user_data["requests"][-1].isoformat()
