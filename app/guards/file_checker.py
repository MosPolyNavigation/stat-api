...
class FileValidator:
    ...
    async def __call__(
        ...
        if (
            content_length
            and int(content_length) > self.max_size * self.request_overhead_multiplier
        ):
            raise HTTPException(
                status_code=413, detail=f"Request too large ({content_length} bytes)"
            )
        ...
        if self.allowed_types and image.content_type not in self.allowed_types:
            raise HTTPException(
                status_code=400, detail=f"Invalid type: {image.content_type}"
            )
...
class SVGValidator:
    ...
    async def __call__(self, file: UploadFile, request: Request) -> UploadFile:
        content_length = request.headers.get("content-length")
        ...
        if self.allowed_types and file.content_type not in self.allowed_types:
            raise HTTPException(
                status_code=400, detail=f"Invalid type: {file.content_type}"
            )
...
image_validator = FileValidator(
    max_size=10 * 1024 * 1024, allowed_types=["image/jpeg", "image/png", "image/gif"]
)

plan_validator = SVGValidator(max_size=1024 * 1024, allowed_types=["image/svg+xml"])

photo_validator = PhotoValidator(
    max_size_per_file=10 * 1024 * 1024, max_total_size=50 * 1024 * 1024, max_files=10
)
