...
async def resolve_dod_nav_floors(
    ...
    name: Optional[int] = None,
) -> list[DodNavFloorType]:
    session = await ensure_nav_permission(info, VIEW_RIGHT_NAME)
    statement = select(DodFloor).order_by(DodFloor.id)
    if id is not None:
        statement = statement.where(DodFloor.id == id)
    if name is not None:
        statement = statement.where(DodFloor.name == name)
