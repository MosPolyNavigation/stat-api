...
from app.models.dod.types import Type as DodNavTypeModel
...
def _to_dod_nav_type(model: DodNavTypeModel) -> DodNavTypeType:
    return DodNavTypeType(id=model.id, name=model.name)


async def resolve_dod_nav_types(info: Info, id: Optional[int] = None, name: Optional[str] = None) -> list[DodNavTypeType]:
    session = await ensure_nav_permission(info, VIEW_RIGHT_NAME)
    statement = select(DodNavTypeModel).order_by(DodNavTypeModel.id)
    if id is not None:
        statement = statement.where(DodNavTypeModel.id == id)
    if name is not None:
        statement = statement.where(DodNavTypeModel.name == name)
...
async def create_dod_nav_type(info: Info, data: DodNavTypeInput) -> DodNavTypeType:
    ...
    nav_type = DodNavTypeModel(name=data.name)
...
async def update_dod_nav_type(info: Info, id: int, data: DodNavTypeUpdateInput) -> DodNavTypeType:
    session = await ensure_nav_permission(info, EDIT_RIGHT_NAME)
    nav_type = await get_or_error(session, DodNavTypeModel, id, "type")
